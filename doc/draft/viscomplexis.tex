...
The four techniques showcased in this paper mark only a tiny spot in the space of possible visualizations concerning proportions structured by age and period. Typically the number of effective visualizations is much smaller than the number of possible visualizations (\cite{Munzner2014}) and therefore a strategy is needed in order to arrive at viable solutions. Our final picks are the result of multiple constraints on the design space: (1) We require the dimensions \emph{time and age to constitute a grid}. This is to be in-line with the Lexis surface plot, an already established visualization tool in demography which highlights patterns along age-, period- and cohort time dimensions. Demographers have build expertise in the interpretation of Lexis surfaces and by extending these to the domain of compositional data we hope to transfer the expertise as well. (2) We require \emph{techniques to be discussed in the literature and/or commonly used to display compositions}. Cartography, statistics and computer science are fields with a strong research agenda on visualization and we believe that demography should form ties with the existing body of knowledge in visualization research. Furthermore, visualizations are easier understood if the users are already familiar with the visual encoding. (3) The techniques have to \emph{differ in their strengths and weaknesses}. While exploring the design space we did not come across a one-size-fits-all solution. Some visualizations work better with a small number of groups, some allow for larger numbers. Some will not work for users with impaired colour vision while some will work flawlessly in black and white print etc. We strive to select a collection of solutions which cover a wide range of use-cases. In order to do so we compare the different visualizations regarding the amount of displayed data, their geometrical preservation of the Lexis surface, their ability to communicate precise values as well as patterns along age, period and cohort, their space economy and their accessibility for users with impaired colour vision. Some evaluation criteria, like the amount of data encoded in the visualization, can be assessed precisely. Other questions, like the ability of the visualization to show patterns in the data, are ultimately a matter of subjective judgements. We back up our personal assessment of these subjective criteria by references to experiments done in graphical perception and by demonstration of the visualizations using real world data.
...
It is also possible to build a colour space on parameters relating to human colour perception. When asked to describe a colour it would be a remarkable feast for a person to express it in terms of the proportions of primary colours. Instead we tend to say that a colour is dark or bright (\emph{lightness}: \testclr{light1} \testclr{light2} \testclr{light3}), very pale or very pure (\emph{chroma}: \testclr{chroma1} \testclr{chroma2} \testclr{chroma3}) and tending to green, blue, red\ldots (\emph{hue}: \testclr{hue2} \testclr{hue3} \testclr{hue1}, see \cite{Fairchild2005} chapter 4 for colour terminology).
...
% Multidimensional colour schemes
...
\begin{figure}[!htb]
  ...
  \caption{Ternary-balance-scheme: Proportion of people dying from a given cause by time and age (France, total population; Data: \cite{Vallin2014}).}
  \label{fig:tbs}
\end{figure}
...
While using colour mixtures to encode multiple data dimensions in a single point has been proposed several times (cp.~\cite{Trumbo1981} and \cite{Eyton1984} for bivariate data on geographical maps; \cite{Ware1988} for cluster identification in high-dimensional spaces) such techniques are not in widespread use. A possible reason is a difficulty in interpretation and the usage of legends which are not easily memorized, criticisms which have been put forward by \cite{Wainer1980}. We seek to alleviate these issues by providing straightforward interpretation guidelines along with a legend that builds upon an already established tool in compositional data analysis -- the ternary diagram.
...
\begin{compactenum}
  ...
  \item the more equal the proportions among the groups are the more the mixed colour tends to grey.
\end{compactenum}
...
\begin{figure}[!htb]
  ...
  \caption{Qualitative-sequential-scheme: The most common cause of death and its share on all deaths by time and age (France, total population; Data: \cite{Vallin2014}).}
  \label{fig:qss}
\end{figure}
