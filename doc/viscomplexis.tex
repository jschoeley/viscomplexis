...
\def\@testclr#1#2{\setlength{\fboxsep}{0pt}\fbox{{\colorbox#1{#2}{\phantom{XX}}}}}
...
It is also possible to build a colour space on parameters relating to human colour perception. When asked to describe a colour it would be a remarkable feast for a person to express it in terms of the proportions of primary colours. Instead we tend to say that a colour is dark or bright (\emph{lightness}: \testclr{light1} \testclr{light2} \testclr{light3}), very pale or very pure (\emph{chroma}: \testclr{chroma1} \testclr{chroma2} \testclr{chroma3}) and tending to blue, red, green\ldots (\emph{hue}: \testclr{hue1} \testclr{hue2} \testclr{hue3}, see \cite{Fairchild2005} chapter 4 for colour terminology).
...
\begin{figure}[htb!]
  \centering
  \includegraphics[width = 0.45\linewidth]{../fig/brewer_subset.pdf}
  \caption{Colour schemes used in this paper (derived from \cite{Brewer1994a}).}
  \label{fig:brewer}
\end{figure}
